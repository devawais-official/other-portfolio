...
import { siteConfig, siteTheme } from "@/lib/site-config";
...
export default function AboutBioCard({ stats, tagline, availability }: AboutBioCardProps) {
    ...
    // --- Clean Translation Fallbacks ---
    const finalTagline = !tagline || tagline.includes("aboutData")
        ? translate("roleVal") || "Senior Mobile Engineer"
        : tagline;

    const finalAvailability = !availability || availability.includes("aboutData")
        ? "Available for freelance"
        : availability;

    const statEntries = [
        { label: translate("stats.experience"), value: stats.yearsExperience },
        { label: translate("stats.completed"), value: stats.projectsCompleted },
        { label: translate("stats.satisfied"), value: stats.clientsSatisfied },
        { label: translate("stats.stores"), value: stats.appsOnStores },
    ];
            ...
            <div>
                ...
                <dl className="mt-5 flex flex-col gap-2.5 text-sm">
                    <div className={style.infoRow}>
                        <dt className={style.infoLabel}>{translate("infoTitle")}</dt>
                        <dd className={style.infoValue}>{translate("infoLocation")}</dd>
                    </div>
                    <div className={style.infoRow}>
                        <dt className={style.infoLabel}>{translate("infoFocus")}</dt>
                        <dd className={style.infoValue}>{translate("infoFocusVal")}</dd>
                    </div>
                </dl>
            </div>
